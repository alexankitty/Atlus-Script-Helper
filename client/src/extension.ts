...
const scriptCompilerArgs: Map<
  string,
  { outFormat: string; library: string; encoding: string }
> = new Map([
...
export function activate(context: ExtensionContext) {
  ...
  client.onReady().then(() => {
    client.onNotification(
      "flowscript/decompileRequired",
      (filePath: string) => {
        if (!ignoreDecompileRequest.includes(filePath)) {
          window.showWarningMessage(
            `${path.basename(filePath)
            } does not appear to be decompiled. Please decompile it to allow messages and functions to be read from it.`,
            `Decompile ${path.basename(filePath)}`,
          ).then((selection) => {
  ...
  const compileFlowscript = (context: any) => {
    ...
    terminal.show(true);
    let toolsPath: string = workspace.getConfiguration("atlusScriptHelper").get(
      "scriptToolsPath",
    );
    let envVars : string = workspace.getConfiguration("atlusScriptHelper").get(
      "environmentVariables",
    );
    let selectedGame: string = workspace.getConfiguration("atlusScriptHelper")
      .get("defaultGame");
    let args = scriptCompilerArgs.get(selectedGame);
    terminal.sendText(
      `${envVars} "${toolsPath}/AtlusScriptCompiler.exe" "${context.fsPath}" -Compile -OutFormat ${args.outFormat} -Library ${args.library} -Encoding ${args.encoding} -Hook`,
      true,
    );
  };
  context.subscriptions.push(
    commands.registerCommand("flowscript.compile", compileFlowscript),
  );
  ...
  const decompileFlowscript = (context: any) => {
    ...
    let args = scriptCompilerArgs.get(selectedGame);
    terminal.sendText(
      `${envVars} "${toolsPath}/AtlusScriptCompiler.exe" "${context.fsPath}" -Decompile -Library ${args.library} -Encoding ${args.encoding} -Hook`,
      true,
    );
  };
  context.subscriptions.push(
    commands.registerCommand("flowscript.decompile", decompileFlowscript),
  );
  ...
  const selectGame = () => {
    window.showQuickPick([
      ...
      "Persona 5 Royal",
      "Persona Q2",
    ], { placeHolder: "Select a compatible game..." })
      .then((selection: string) => {
        workspace.getConfiguration("atlusScriptHelper").update(
          "defaultGame",
          selection,
  ...
  context.subscriptions.push(
    commands.registerCommand("flowscript.selectGame", selectGame),
  );
